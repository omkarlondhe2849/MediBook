...
public class ReviewDAO {
    ...
    public List<Review> findAll() {
        logger.debug("Executing query: SELECT * FROM doctor_reviews ORDER BY id DESC");
        try {
            String sql = "SELECT id, patient_id as userId, doctor_profile_id as serviceId, rating, review_comment as comment, created_at as createdAt FROM doctor_reviews ORDER BY id DESC";
    ...
    public List<Review> findAllByServiceId(Long serviceId) {
        logger.debug("Executing query: SELECT * FROM doctor_reviews WHERE doctor_profile_id = {}", serviceId);
        try {
            String sql = "SELECT id, patient_id as userId, doctor_profile_id as serviceId, rating, review_comment as comment, created_at as createdAt FROM doctor_reviews WHERE doctor_profile_id = ?";
    ...
    public int save(Review review) {
        ...
        try {
            String sql = "INSERT INTO doctor_reviews(patient_id, doctor_profile_id, rating, review_comment) VALUES (?, ?, ?, ?)";
    ...
    public Double getAverageRatingForProvider(Long providerId) {
        logger.debug("Executing query: SELECT AVG(r.rating) FROM doctor_reviews r JOIN doctor_profiles s ON r.doctor_profile_id = s.id WHERE s.doctor_id = {}", providerId);
        try {
            String sql = "SELECT AVG(r.rating) FROM doctor_reviews r " +
                        "JOIN doctor_profiles s ON r.doctor_profile_id = s.id " +
                        "WHERE s.doctor_id = ?";
    ...
    public Integer getReviewCountForProvider(Long providerId) {
        logger.debug("Executing query: SELECT COUNT(r.id) FROM doctor_reviews r JOIN doctor_profiles s ON r.doctor_profile_id = s.id WHERE s.doctor_id = {}", providerId);
        try {
            String sql = "SELECT COUNT(r.id) FROM doctor_reviews r " +
                        "JOIN doctor_profiles s ON r.doctor_profile_id = s.id " +
                        "WHERE s.doctor_id = ?";
    ...
    public Double getPlatformAverageRating() {
        logger.debug("Executing query: SELECT AVG(rating) FROM doctor_reviews");
        try {
            String sql = "SELECT AVG(rating) FROM doctor_reviews";
